...
fun validateOnly(baseUrl: String, apiKey: String, listing: Listing): JSONObject {
    ...
    val obj = JSONObject()
        ...
        .put("policy_review", JSONObject()
            .put("publish_allowed", listing.publishAllowed)
            .put("block_reason", listing.policyBlockReason)
            .put("warnings", JSONArray(listing.policyWarnings)))
    ...
    fun publish(baseUrl: String, apiKey: String, listings: List<Listing>): List<PublishResult> {
        listings.firstOrNull { !it.publishAllowed }?.let {
            throw IllegalArgumentException("Policy block for ${it.sku}: ${it.policyBlockReason.ifBlank { "not approved for publication" }}")
        }
        val arr = JSONArray()
        listings.forEach { x ->

...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val savedWorkflow = WorkflowStore.load(this)
        batchId = savedWorkflow.batchId
        b.workflowVersionText.text = "Fixed workflow ${savedWorkflow.version} • Resumes automatically"

        b.runWorkflowButton.setOnClickListener { runWorkflow() }
        ...
        b.importButton.setOnClickListener { openBatchPicker() }
    ...
    private fun ensureBatchId(): String {
        if (batchId.isBlank()) batchId = UUID.randomUUID().toString().substring(0, 8)
        return batchId
    }

    private fun transition(stage: WorkflowStage, message: String = "") {
        WorkflowStore.transition(this, stage, ensureBatchId(), message)
    }

    private fun openBatchPicker() {
    ...
    private fun runWorkflow() {
        if (!baseUrl().startsWith("https://") || apiKey().isBlank()) {
            transition(WorkflowStage.ERROR, "Secure service connection is not configured")
            toast("First enter the Google Cloud service address and app access key, then tap Connect securely.")
            return
        }
        if (listings.isEmpty()) {
            transition(WorkflowStage.IMPORTING, "Waiting for the desktop photo or listing ZIP")
            openBatchPicker()
            return
        }
        if (WorkflowStore.load(this).stage == WorkflowStage.WAITING_FOR_CHATGPT) {
            openBatchPicker()
            return
        }

        applyDefaults()
        val factualErrors = listings.flatMap { listing ->
            listing.errors.filter {
                it == "No photos" || it.startsWith("Missing photo:")
            }
        }
        if (factualErrors.isNotEmpty()) {
            transition(WorkflowStage.DETAILS_REQUIRED, factualErrors.first())
            val target = listings.indexOfFirst { listing ->
                listing.errors.any { error -> error in factualErrors }
            }.takeIf { it >= 0 } ?: 0
            toast("Add the missing product photos. ChatGPT will research the details and assess visible condition.")
            editListing(target)
            return
        }

        if (listings.any { it.marketResearchCheckedAt.isBlank() || it.comparableSources.isEmpty() }) {
            shareForChatGptResearch()
            return
        }

        val unresolved = listings.flatMap { it.errors }
        if (unresolved.any { it.startsWith("Missing eBay aspect:") }) {
            transition(WorkflowStage.REVIEW_REQUIRED, "Required eBay fields need confirmation")
            fillMissingAspects()
            return
        }
        if (unresolved.isNotEmpty()) {
            transition(WorkflowStage.REVIEW_REQUIRED, unresolved.first())
            toast("Review the highlighted issue before publishing: ${unresolved.first()}")
            return
        }

        transition(WorkflowStage.READY_TO_PUBLISH, "Final approval required")
        AlertDialog.Builder(this)
            .setTitle("Ready to publish")
            .setMessage("${listings.count { it.selected && it.ready }} selected listing(s) passed the fixed workflow. Review price and shipping, then approve publication.")
            .setPositiveButton("Publish to eBay") { _, _ ->
                transition(WorkflowStage.PUBLISHING, "Publishing approved listings")
                publishItems(listings.filter { it.selected && it.ready && it.publishState != PublishState.PUBLISHED })
            }
    ...
    private fun shareForChatGptResearch() {
        listings.forEach { it.researchRequired = true; BatchImporter.validateBase(it) }
        val id = ensureBatchId()
        try {
            val uri = ChatGptResearchPackage.create(this, listings, id)
            val share = Intent(Intent.ACTION_SEND).apply {
                type = "application/zip"
                putExtra(Intent.EXTRA_STREAM, uri)
                putExtra(Intent.EXTRA_SUBJECT, "Research VEbalist batch $id")
                putExtra(Intent.EXTRA_TEXT, "Use the instructions inside this VEbalist package. Return VEbalist-Ready-$id.zip.")
                clipData = android.content.ClipData.newRawUri("VEbalist research package", uri)
                addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
            }
            transition(WorkflowStage.WAITING_FOR_CHATGPT, "Waiting for the completed ChatGPT ZIP")
            startActivity(Intent.createChooser(share, "Send research package to ChatGPT"))
        } catch (e: Exception) {
            recordError("Create ChatGPT research package", e.message)
            toast("Unable to prepare the ChatGPT package: ${e.message}")
        }
    }

    private fun handleIncomingZip(incoming: Intent?) {
        if (incoming?.action != Intent.ACTION_VIEW) return
        val uri = incoming.data ?: return
        try {
            listings = BatchImporter.importZip(this, uri).toMutableList()
            if (listings.isEmpty()) throw IllegalArgumentException("The returned package contains no listings.")
            transition(WorkflowStage.REVIEW_REQUIRED, "ChatGPT research package imported")
            show()
            toast("ChatGPT research imported. Tap Continue workflow to validate and publish.")
        } catch (e: Exception) {
            recordError("Open returned ChatGPT package", e.message)
            toast("Unable to import the returned package: ${e.message}")
        } finally {
    ...
    override fun onActivityResult(requestCode:Int,resultCode:Int,data:Intent?){
        ...
        try{
            listings=BatchImporter.importZip(this,data?.data?:return).toMutableList()
            ensureBatchId()
            transition(
                if (listings.any { !it.ready }) WorkflowStage.DETAILS_REQUIRED else WorkflowStage.REVIEW_REQUIRED,
                "Batch imported"
            )
            show()
        }
    ...
    private fun buildErrorLog(): String = buildString {
        ...
        appendLine("Created: ${java.util.Date()}")
            ...
            appendLine("Workflow version: ${WorkflowStore.CURRENT_WORKFLOW_VERSION}")
            appendLine("Workflow stage: ${WorkflowStore.load(this@MainActivity).stage}")
            appendLine("Batch ID: ${batchId.ifBlank { "Not started" }}")
        ...
        listings.forEachIndexed { index,x ->
            ...
            if (x.marketResearchCheckedAt.isNotBlank()) {
                appendLine("   Comparable research checked: ${x.marketResearchCheckedAt}")
                appendLine("   Comparable range: ${x.comparablePriceLow ?: "n/a"} to ${x.comparablePriceHigh ?: "n/a"}")
                appendLine("   Comparable sources: ${x.comparableSources.size}")
            }
            if (!x.publishAllowed) appendLine("   POLICY BLOCK: ${x.policyBlockReason}")
            x.policyWarnings.forEach { appendLine("   Policy warning: $it") }
        ...
        val improvements = WorkflowStore.improvements(this@MainActivity)
        appendLine()
        appendLine("Workflow improvement log: ${improvements.size} issue(s)")
        improvements.takeLast(20).forEach {
            appendLine("- ${java.util.Date(it.createdAt)} [${it.stage}] ${it.issue}")
        }
    }
    ...
    private fun publishItems(items:List<Listing>){
        ...
        thread{
            ...
            runOnUiThread{
                when {
                    listings.any { it.publishState == PublishState.FAILED } ->
                        transition(WorkflowStage.ERROR, "One or more listings failed to publish")
                    listings.filter { it.selected }.all { it.publishState == PublishState.PUBLISHED } ->
                        transition(WorkflowStage.COMPLETE, "Selected listings published")
                    else -> transition(WorkflowStage.REVIEW_REQUIRED, "Review remaining listings")
                }
                show()
            }
        }
    ...
    private fun show(){
        ...
        val workflow = WorkflowStore.load(this)
        b.runWorkflowButton.text = when (workflow.stage) {
            WorkflowStage.IDLE, WorkflowStage.COMPLETE -> "Create and publish listings"
            WorkflowStage.WAITING_FOR_CHATGPT -> "Import completed ChatGPT ZIP"
            WorkflowStage.PUBLISHING -> "Publishing…"
            else -> "Continue workflow"
        }
        b.runWorkflowButton.isEnabled = workflow.stage != WorkflowStage.PUBLISHING
    ...
    private fun recordError(action:String, message:String?) {
        ...
        val stage = WorkflowStore.load(this).stage
        WorkflowStore.recordIssue(this, stage, "$action — $safeMessage")
        transition(WorkflowStage.ERROR, safeMessage)
        if (::b.isInitialized) {

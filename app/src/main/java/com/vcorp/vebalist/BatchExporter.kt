...
object BatchExporter {
    fun write(context: Context, uri: Uri, listings: List<Listing>) {
        ...
        val batch = JSONObject()
            ...
            .put("currency",BatchContract.DEFAULT_CURRENCY)
    ...
    private fun listingJson(x: Listing): JSONObject {
        ...
        val research=JSONObject()
            .put("checked_at",x.marketResearchCheckedAt)
            .put("comparable_price_low",x.comparablePriceLow ?: JSONObject.NULL)
            .put("comparable_price_high",x.comparablePriceHigh ?: JSONObject.NULL)
            .put("sources",JSONArray(x.comparableSources))
            .put("notes",x.researchNotes)
        val policy=JSONObject()
            .put("publish_allowed",x.publishAllowed)
            .put("block_reason",x.policyBlockReason)
            .put("warnings",JSONArray(x.policyWarnings))
        return JSONObject()
            ...
            .put("market_research",research).put("policy_review",policy)
    }
}

...
Current Android release: **2.0.5 (205)** using fixed workflow **1.0**.

The main **Create and publish listings** button now runs or resumes the complete listing sequence. Photo editing remains a separate free desktop step. Research is handed to the signed-in ChatGPT Android app through a ZIP package, so no OpenAI API key is embedded in VEbalist. See `WORKFLOW_HANDOFF.md` for the implemented state and continuation instructions.
